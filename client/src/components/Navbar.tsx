        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href}>
                <a className="text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors duration-200 relative group">
                  {item.label}
          ...
          <div className="hidden md:flex items-center gap-4">
            <a
              ...
              className="px-6 py-2.5 bg-yellow-500 text-white rounded-lg font-semibold hover:bg-yellow-600 transition-all duration-200 active:scale-95 shadow-sm hover:shadow-md"
            >
              Book Consultation
            </a>
          </div>
            ...
            >
              <div className="px-4 py-4 space-y-2">
                {navItems.map((item) => (
                  <Link key={item.href} href={item.href}>
                    <a
                      ...
                      className="block px-4 py-2.5 text-gray-600 hover:bg-gray-50 hover:text-yellow-600 rounded-lg transition-colors"
                    >
                      {item.label}
                    </a>
                  </Link>
                ...
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={() => setIsOpen(false)}
                  className="block w-full mt-4 px-4 py-2.5 bg-yellow-500 text-white rounded-lg font-semibold text-center hover:bg-yellow-600 transition-all"
                >
                  Book Consultation
                </a>
              </div>
